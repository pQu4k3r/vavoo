...
class VavooProxy:
            ...
            while not self._stop_event.is_set():
                ...
                try:
                    ...
                    if self.addon_sig_data["sig"] and token_age > TOKEN_REFRESH_AGE:
                        print("[Token Monitor] Token old ({}s), refreshing...".format(
                            int(token_age)))
    ...
    def refresh_addon_sig_if_needed(self, force=False):
        ...
        """
        # Lazy-init the done-event (not in __init__ to avoid changing __init__ signature)
        ...
        # If a refresh is already in progress, wait for it then return its result.
                            ...
                            if chunk:
                                try:
                                    ...
                                    self.wfile.flush()
                                        ...
                                        ConnectionResetError):
                                    # Client disconnected – stop forwarding silently
                                    print("[Proxy Stream] Client disconnected for: " + channel_id)
                                    break
                                last_data_time = time.time()
                            else:
                                if time.time() - last_data_time > 15:
                                    print("[Proxy Stream] Upstream stalled for: " + channel_id)
                                    break
                                select.select([], [], [], 0.1)
                ...
                response_channels = [
                    {
                        "id": ch.get("id", ""),
                        "name": ch.get("name", ""),
                        "url": "http://%s:%d/vavoo?channel=%s" % (local_ip, PORT, ch.get("id", "")),
                        "logo": ch.get("logo", ""),
