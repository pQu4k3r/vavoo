...
class MainVavoo(Screen):
    def __init__(self, session):
        ...
        if cfg.proxy_enabled.value:
            ...
            self.proxy_monitor_timer.start(30000)  # 30s is sufficient; 10s was too aggressive
    ...
    def preload_flags_for_visible_countries(self):
        ...
        try:
            ...
            print("[MainVavoo] Preloading flags for %d countries (background)" %
                  len(countries_list))
            ...
            def _download_all():
                ...
                for i, country in enumerate(countries_list):
                    ...
                    # After first 8, trigger a UI refresh so visible flags appear quickly
    ...
    def ok(self):
        try:
            ...
            if not current_item or len(current_item) == 0:
                debug("No current item selected or item is empty", area="PLUGIN")
                return
...
class vavoo(Screen):
    ...
    def cat(self):
        ...
        debug("vavoo.cat() called for country: " + str(self.country_name), area="PLUGIN")
        ...
        if not cfg.proxy_enabled.value:
            ...
            def _done(result):
                if result:
                    ...
                    debug("Proxy returned empty response, trying fallback...", area="PLUGIN")
                    self._fallback_to_original_method()
    ...
    def _build_channel_list(self, channels_data):
        ...
        self.itemlist = [item[0][0] + "###" + item[0][1] for item in self.cat_list]
        self.update_menu()
        debug("List built with " + str(len(self.cat_list)) + " items.", area="PLUGIN")
    ...
    def _on_export_complete(self, success, ch_count, message):
        ...
        try:
            ...
            if message == "Bouquet created":
                ...
                debug("Bouquet ready with {} channels".format(ch_count), area="PLUGIN")
                if NOTIFICATION_AVAILABLE:
                    quick_notify(
                ...
                debug("EPG completed for {} channels".format(ch_count), area="PLUGIN")
                if NOTIFICATION_AVAILABLE:
                    if ch_count > 0:
                        quick_notify(
...
class Playstream2(
    ...
    def get_current_epg(self):
        ...
        try:
            ...
            if cached and (time.time() - cached[0]) < _XML_CACHE_TTL:
                ...
                if fetch_time > 0.5:
                    print("[EPG] Slow fetch {}: {:.3f}s".format(epg_url, fetch_time))
                if not xml_data:
    ...
    def cancel(self):
        ...
        try:
            for screen in self.session.dialog_stack:
                if hasattr(screen[0], 'proxy_monitor_timer'):
                    screen[0].proxy_monitor_timer.start(30000)  # consistent with main 30s interval
